...
the `startTabwrightCDPRelayServer` closure:
...
(`startTabwrightCDPRelayServer` return type, HTTP endpoints, WebSocket protocol)
...
top of the `startTabwrightCDPRelayServer` closure, replacing the separate
