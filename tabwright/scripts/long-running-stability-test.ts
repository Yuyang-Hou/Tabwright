/**
 * Long-running Tabwright stability test.
 *
 ...
 *
 * Usage:
 *   bun tabwright/scripts/long-running-stability-test.ts
 *   bun tabwright/scripts/long-running-stability-test.ts --duration 2  # hours (default: 4)
 */
...
async function pw(sessionId: string, code: string): Promise<string> {
  const result =
    await $`tabwright -s ${sessionId} --timeout 30000 -e ${code}`.text()
  return result.trim()
}
...
async function main(): Promise<void> {
  ...
  const sessionRaw = await $`tabwright session new`.text()
