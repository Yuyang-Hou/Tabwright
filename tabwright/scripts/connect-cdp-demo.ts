 ...
 * Tabwright relay via the CDP URL on localhost:19988.
 *
 * Prereqs:
 *   1. `tabwright serve --token secret_token --replace` is running
 *   2. The Tabwright Chrome extension is connected (toggled on a tab)
 *
 * Run:
 *   tsx connect-cdp.ts
 */
...
if (!TOKEN) {
  throw new Error('Set PLAYWRITER_TOKEN env var to the token you passed to `tabwright serve --token …`.')
}
...
async function main(): Promise<void> {
  ...
  if (pages.length === 0) {
    console.error('No pages — toggle the Tabwright extension on a tab first.')
    await browser.close()
    process.exit(1)
  }
