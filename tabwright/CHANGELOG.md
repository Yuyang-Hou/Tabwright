...
- 6b8e8d9: Remove the legacy video recording product path, including extension capture permissions, offscreen capture files, video replay UI, relay storage, and `recording.*` execute-context APIs. Replay recording now uses rrweb DOM capture only.

### Minor Changes

- 60b6b6b: Record capability contract conformance, detect undeclared network hosts, downgrade trusted capabilities on drift, and expose lifecycle evidence.
- 9ae9d89: Negotiate extension and relay features without blocking core browser control when optional APIs or versions differ.
- 04801ef: Add capability agent skill lifecycle commands so agents can scaffold, inspect, and install capability-specific Codex skills without pretending the CLI authored the final instructions.
- 8cc09cb: Add `tabwright doctor` with human and JSON output so users and agents can diagnose relay, extension, enabled-tab, session, and capability readiness from one command and receive a single concrete next step.
- 6b8e8d9: Add a built-in `conan-config` capability suite for installing read-only Conan/Buff copywriting config search and query capabilities that refresh cookie auth from the user's current Chrome session.
  ...
  Add a lightweight `tabwright capability route` command and MCP route action so agents can resolve exact-match direct-run capabilities without first reading full Tabwright docs, searching, or describing contracts.
  ...
  Include sandbox execution hints in exact-match route results so Codex agents know to run trusted capability commands with escalated permissions when they need to write run logs or artifacts under `~/.tabwright`.

  Install bundled agent skills alongside built-in capability suites so capability-specific instructions live next to the capability instead of being hard-coded into the general Tabwright skill.

- Add capability authentication status and a user-confirmed browser cookie refresh flow for the extension Options page, and bundle the matching Tabwright agent skill with explicit install and status commands.
- Publish the fork as Tabwright with the `tabwright` CLI, bundled Codex skill, Tabwright-branded extension, and compatibility for existing Playwriter environments and user data.
- 6b8e8d9: Add replay AI indexes that summarize rrweb recordings into actions, fields, selector hints, and page context for faster workflow compilation.
- 18fbcde: Add local replay discovery, compact evidence output, and structured `compiled` or `needs_ai` handoffs so agents can turn recordings into confirmed browser capabilities without inventing automation for unsupported workflows.
- 6b8e8d9: Add recording-time replay annotations so users can mark important elements while recording. Annotation targets are highlighted while users write the note, saved annotations remain visible as compact callouts, and users can hover a callout to delete it. Annotation add/delete events are stored in rrweb and reflected in replay AI indexes.
- 6b8e8d9: Add an rrweb replay compiler that turns supported demonstrations into runnable browser workflow capabilities with `needs_ai` handoff on validation or page drift. Includes `tabwright replay make` for the common replay-to-capability flow and a `tabwright replay eval` self-test platform that creates local example pages, indexes recordings, compiles capabilities, runs them in a real browser, and verifies the result.
- 6b8e8d9: Add rrweb-backed DOM replay recording/playback APIs, saved replay listing, and extension options playback.
- 9b86ca9: Add safe capability packaging and installation from directories, local `.tgz` archives, and HTTPS `.tgz` URLs. Shared packages exclude local secrets, run logs, and artifacts, and always install as drafts.
- Add saved Tabwright capabilities with CLI commands, MCP access, AI-readable contracts, node runtime support, auth refresh, and a local studio for managing reusable browser and API tools.
- 9b86ca9: Add operation-level capability policies so one saved capability can expose autonomous read operations and separately confirmed write operations. Replace the built-in Conan config search/query capabilities with one unified domestic production/test capability for search, read, schema and history inspection, validated content changes, drafts, publishing, config creation, definition copying, and verified rollback.

### Patch Changes

- 1e25227: Enforce `requiresConfirmation` for node, browser, CLI, and MCP capability runs with an exact-id confirmation flag that `--force` cannot bypass, generate replay/browser run commands with the correct user-browser and confirmation arguments, clear completed node-capability timeout timers so successful CLI runs exit immediately, and replace the mandatory 17k-token reference read with a compact browser core protocol plus topic queries.
- 4224ad1: Ignore common English stop words when matching capability search intent so unrelated capabilities, including side-effecting ones, are not returned merely because their metadata contains words such as `a`, `the`, or `to`.
- c363a97: Restart same-version relay processes that are missing current feature endpoints, and keep saved capability listings available when a script or run-log entry is damaged.
- 7406d23: Add a read-only capability view to the extension options page with copyable AI prompts for use, edits, and agent skill updates. The extension toolbar now also shows a recovery warning when the local Tabwright service is missing or too old.
- 2555d7c: Expire previously responsive extension connections after missed heartbeats so half-open sockets stop appearing connected and existing cleanup can reconnect the browser promptly.
- 60b118d: Improve local relay connection handling across CLI, MCP, capabilities, and recording by trying multiple loopback hosts before treating the relay as unavailable.
- d87c46a: Wait for the current or a newer compatible relay package version after restart, let reconnecting Chrome profiles settle, and automatically select only the unambiguous profile with enabled tabs.
- 7406d23: Keep DOM replay checkouts disabled by default so cross-origin iframe content stays attached to the correct replay frame.
- 6b8e8d9: Allow in-page toolbar rrweb recordings to be saved, indexed, and reviewed from the extension options page. Stopping a toolbar recording now also copies an AI handoff prompt with the replay id and replay make/run commands.
- 60b118d: Show the `tabwright --help` hint when an unknown root command is used.
- 6b8e8d9: Generate recording-derived workflows as direct runnable scripts that return AI handoff context on page drift.
...
1. **Make `browser start` work from source checkouts** — runtime package path resolution now falls back to the local `playwriter/` package directory before using installed-package resolution, so `tsx tabwright/src/cli.ts browser start` works during development.
