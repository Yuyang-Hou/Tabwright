...
import { getCdpUrl, parseRelayHost, shouldAutoEnableTabwright } from './utils.js'
...
1. Installed the extension: https://chromewebstore.google.com/detail/tabwright/jfeammnjpkecdekppnclgkkffahnhfhe
...
export class PlaywrightExecutor {
  ...
  private setupPageCloseDetection(page: Page) {
    page.on('close', () => {
      ...
      this.enqueueWarning(
        `The current page in ${stateKeyLabel} was closed (url: ${closedUrl}). ` +
          `No open pages remain. Open a tab with Tabwright enabled, then reassign ${stateKeyLabel}.`,
      )
    })
  }
  ...
  private async checkExtensionStatus(): Promise<{
    ...
    const effectiveToken = token || process.env.TABWRIGHT_TOKEN || process.env.PLAYWRITER_TOKEN
    if (effectiveToken) {
    ...
    try {
      ...
      const screenshotWithAccessibilityLabelsFn = async (options: { page: Page; interactiveOnly?: boolean }) => {
        return screenshotWithAccessibilityLabels({
          ...
          logger: {
            info: (...args) => {
              this.logger.error('[tabwright]', ...args)
            },
            error: (...args) => {
              this.logger.error('[tabwright]', ...args)
            },
