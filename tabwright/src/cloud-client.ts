...
//   1. TABWRIGHT_API_KEY env var → sent as x-api-key header (for CI/VPS/headless)
//   2. TABWRIGHT_CLOUD_TOKEN env var → sent as Authorization: Bearer (for CI with session tokens)
//   3. ~/.tabwright/auth.json file → saved by `cloud login` device flow
...
const AUTH_FILE = path.join(getTabwrightUserDataDir(), 'auth.json')

/** Build the hosted live-view URL from the exact CDP WebSocket URL.
...
export function loadCloudAuth(): CloudAuth | null {
  ...
  const apiKey = process.env.TABWRIGHT_API_KEY || process.env.PLAYWRITER_API_KEY
  if (apiKey) {
    return {
      token: apiKey,
      baseUrl: process.env.TABWRIGHT_CLOUD_URL || process.env.PLAYWRITER_CLOUD_URL || DEFAULT_BASE_URL,
      isApiKey: true,
    }
  }
  // Session token env var (for CI with device flow tokens)
  const envToken = process.env.TABWRIGHT_CLOUD_TOKEN || process.env.PLAYWRITER_CLOUD_TOKEN
  if (envToken) {
    return {
      token: envToken,
      baseUrl: process.env.TABWRIGHT_CLOUD_URL || process.env.PLAYWRITER_CLOUD_URL || DEFAULT_BASE_URL,
...
export class CloudClient {
  ...
  private async request<T>(
    ...
    if (response.status === 401) {
      throw new Error('Cloud auth expired or invalid. Run `tabwright cloud login` or set TABWRIGHT_API_KEY.')
    }
