...
<html>
  ...
  it('should expose CDP discovery endpoints /json/version and /json/list', async () => {
    ...
    expect(versionJson).toMatchObject({
      Browser: expect.stringContaining('Tabwright/'),
  ...
  it('should not crash when page has chrome-extension:// iframe from another extension', async () => {
    ...
    // When Tabwright attaches the debugger and Target.setAutoAttach is active, Chrome
    ...
    const tabwrightExtId = serviceWorker.url().match(/chrome-extension:\/\/([^/]+)/)?.[1]
    expect(tabwrightExtId).toBeTruthy()
    ...
    try {
      ...
      // Enable Tabwright on this page — this must NOT crash the debugger
      await withTimeout({
        promise: serviceWorker.evaluate(async () => {
          await globalThis.toggleExtensionForActiveTab()
        }),
