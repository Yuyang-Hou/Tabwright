...
export function buildDoctorReport(options: {
      ...
      return {
        ...
        detail:
          ...
          (options.remote
            ...
            : 'Tabwright could not start or reach the relay. Read the relay log for the underlying error.'),
      }
    }
  ...
  const extensionCheck: DoctorCheck = (() => {
    ...
    if (activeTargets === 0) {
      return {
        ...
        detail: 'Open the target tab and click the Tabwright extension icon until it turns green.',
      }
    }
  ...
  const next: DoctorNextStep = (() => {
    ...
    if (options.extensions.length > 0) {
      return {
        title: 'Open the target tab and click the Tabwright extension icon until it turns green.',
        fallbackCommand: 'tabwright session new --browser headless',
      }
    }
...
export function formatDoctorReport(report: DoctorReport): string {
  ...
  return [
    `Tabwright doctor ${report.version}`,
