...
export const RELAY_PORT = Number(process.env.TABWRIGHT_PORT || process.env.PLAYWRITER_PORT) || 19988
...
/**
 * Check if the running Tabwright package is older than the version the extension was built with.
 * The extension bundles the Tabwright version at build time. If the extension reports a newer
 * version, it means the user's CLI/MCP needs updating.
 * Returns a warning message if outdated, null otherwise.
 */
export function getExtensionOutdatedWarning(extensionTabwrightVersion: string | null | undefined): string | null {
  if (!extensionTabwrightVersion) {
    return null
  }
  if (compareVersions(extensionTabwrightVersion, VERSION) > 0) {
    return `Tabwright ${VERSION} is outdated (extension requires ${extensionTabwrightVersion}). Run \`npm install -g tabwright@latest\` or update the tabwright package in your project.`
  }
  return null
}
