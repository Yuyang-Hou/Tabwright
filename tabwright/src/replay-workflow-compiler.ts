...
function buildListAppendScript(analysis: ReplayWorkflowAnalysis, options: CompileReplayWorkflowOptions): string {
  ...
  return [
    ...
    '}',
    '',
    'async function humanCheckpoint(phase) {',
    '  const bodyText = await page.locator("body").innerText({ timeout: 1000 }).catch(() => "");',
    '  const challengeText = bodyText.match(/安全验证|请完成验证|滑块验证|扫码验证|短信验证|verification required|verify you are human|captcha/i)?.[0];',
    '  const challengeFrameCount = await page.locator("iframe[src*=\\"captcha\\" i]").count().catch(() => 0);',
    '  if (!challengeText && challengeFrameCount === 0) return undefined;',
    '  return {',
    '    status: "needs_human",',
    '    reason: "The website requires a person to complete verification in the user browser.",',
    '    phase,',
    '    challenge: challengeText || "captcha iframe",',
    '    resume: "Complete verification in the open browser, then run the same capability again.",',
    '    context: { url: page.url(), title: await page.title().catch(() => "") },',
    '  };',
    '}',
    '',
    ...
    '    }',
    '  }',
    '  const checkpoint = await humanCheckpoint(phase);',
  ...
  const saved = saveWorkflowCapability({
    ...
    execution: {
      ...
      observedRequestPatterns: [
        '*/enhancedConfigs/update*',
        '*/enhancedConfigs/publish*',
        '*/enhancedConfigs/detail*',
      ],
    },
  })
