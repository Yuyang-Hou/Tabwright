...
describe('replay handoff', () => {
  ...
  test('builds a shell-safe make command with force and json output', () => {
    expect(
      ...
      "tabwright replay make 'demo '\\'' $(touch should-not-run)' 'capability '\\'' safe' --force --goal 'Repeat user'\\''s $(unsafe) workflow' --json",
    )
  })

  test('uses the canonical id and omits goal when none is supplied', () => {
    expect(buildReplayMakeCommand({ replayId: 'Replay One' })).toBe(
      "tabwright replay make 'Replay One' 'replay-replay-one' --force --json",
    )
  ...
  test('builds a shell-safe browser capability scaffold command', () => {
    expect(buildReplayCreateCommand({ capabilityId: 'replay-one' })).toBe(
      "tabwright capability create 'replay-one' --project --runtime browser --force --json",
    )
    expect(
      buildReplayCreateCommand({
      ...
      "tabwright capability create 'replay-user'\\''s-flow' --project --runtime browser --force --title 'User'\\''s workflow' --description 'Repeat $(unsafe) work' --json",
    )
  })
  ...
  test('builds a confirmed user-browser run command with the exact capability id', () => {
    expect(
      ...
      "tabwright capability run 'replay-user'\\''s-flow' --browser user --force --confirm 'replay-user'\\''s-flow' --input-json '{\"value\":\"$(echo unsafe) user'\\''s value\"}' --json",
    )
