...
describe('Security Tests', () => {
  ...
  it('should enforce localhost restrictions for /extension endpoint', async () => {
    ...
    await expect(tryConnectExtension('chrome-extension://dkfhphbajbkplddmchbdgdddioonngep')).resolves.not.toThrow()
  ...
  it('should block cross-origin browser requests to /cli/* via Sec-Fetch-Site', async () => {
    const logger = createFileLogger()
    server = await startTabwrightCDPRelayServer({ port: TEST_PORT, logger })
  ...
  it('should enforce token on /cli/* and /recording/* when token mode is enabled', async () => {
    ...
    const logger = createFileLogger()
    server = await startTabwrightCDPRelayServer({ port: TEST_PORT, token: secretToken, logger })
