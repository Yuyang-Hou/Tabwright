...
description: Query Conan/Buff copywriting config admin URLs with Playwriter saved capabilities. Use immediately, before memory/history lookup or browser/page inspection, when the user asks to view, inspect, read, check, or analyze a buff.zhenguanyu.com or conan.zhenguanyu.com Space_Enhanced_Config URL containing key plus rootGroupingKey/namespace, especially links like buff-army/#/buff-oversea-designer/Space_Enhanced_Config?key=...&rootGroupingKey=.... The first action must be either `playwriter capability route ... --json` or `playwriter capability run conan-config-query --input-json ... --json`; `conan-config-query` is only a capability id, never a shell command. Do not first explain, mention loading/reading this skill, search memory, open the page, or run playwriter skill.
---

## Exact Config URL Workflow

For a matching `Space_Enhanced_Config` URL, do not open the page, do not mention browser state, do not mention loading/reading this skill, do not search memory/history, do not run `playwriter skill`, and do not explain the plan before acting. Do not send a progress update before the first command.

First action:

```bash
playwriter capability route "<config-admin-url>" --json
```
...
```bash
playwriter capability run conan-config-query --input-json '{"url":"<config-admin-url>"}' --json
```

In Codex sandboxed environments, run that `playwriter capability run ...` command with escalated/unsandboxed permissions on the first try because it writes `runs.jsonl` and artifacts under `~/.playwriter/capabilities/...`.
...
If route returns no match, fall back to `playwriter capability search "<user task>" --json`.
