...
This checks the relay, extension version, enabled tabs, sessions, and visible capabilities. Follow the single `Next` action it prints before manually restarting processes. For agent or support automation, use `tabwright doctor --json`.
...
# Find errors
grep -i error ~/.tabwright/relay-server.log | tail -20
```
...
3. Is the relay server running? The CLI starts it automatically, but if you killed it manually, run `tabwright serve` or just use any CLI command to restart it

### "No browser tabs have Tabwright enabled"
...
```bash
tabwright -s 1 --timeout 30000 -e 'await page.goto("https://slow-site.com")'
```
...
# Or use --replace flag
tabwright serve --token MY_SECRET --replace
```
