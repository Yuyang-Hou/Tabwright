---
title: Tabwright vs Claude Browser Extension
...
Tabwright works with **any MCP client** (Claude, Cursor, OpenCode, Windsurf, and more) and
...
Tabwright's accessibility snapshots are **5-20KB of text** that contain every interactive
element with ready-to-use locators. The agent can parse them without vision, which is
faster and cheaper.

When spatial layout matters (dashboards, image galleries), Tabwright offers
...
The Claude extension only works with Claude. Tabwright works with any tool that supports
...
tabwright -s 1 -e "page.locator('button:has-text(\"Submit\")').click()"
```

## Advanced capabilities

Tabwright exposes the full Chrome DevTools Protocol. Set breakpoints, step through code,
