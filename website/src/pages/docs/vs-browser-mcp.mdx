...
Tabwright has **one tool**: `execute`. It runs any Playwright code. The agent already knows
...
tokens the agent pays on every request just to know what's available. Tabwright's single
...
you're stuck. Tabwright lets agents run **any Playwright code**, plus raw CDP commands
...
# Tabwright: anything Playwright can do

tabwright -s 1 -e "page.evaluate(() => performance.getEntriesByType('navigation'))"
tabwright -s 1 -e "state.cdp = getCDPSession({ page }); state.cdp.send('Network.enable')"
```
