---
title: Tabwright vs Chrome Direct CDP
sidebarTitle: vs Chrome Direct CDP
description: Why Chrome's built-in CDP remote debugging is impractical for agents, and how Tabwright solves it.
...
## How Tabwright avoids the dialog

Tabwright uses a Chrome extension with `chrome.debugger` permissions instead of external CDP
...
With Tabwright's `--direct` mode, you can still use Chrome's built-in CDP when you
...
With Tabwright, the user enables the extension on the tabs they want controlled, then walks
