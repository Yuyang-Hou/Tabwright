...
async function requestJson(path) {
  ...
  if (!cookieHeader) {
    throw new Error("Missing saved cookie auth. Run: playwriter capability refresh-auth conan-config --browser user --json");
  }
  ...
  if (response.status === 401 || response.status === 403) {
    throw new Error("Request failed " + response.status + ": login expired. Run: playwriter capability refresh-auth conan-config --browser user --json");
  }
